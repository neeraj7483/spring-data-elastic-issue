...
import com.demo.issue.custom.EntityOperations.AdaptibleEntity;
 ...
 * @since 3.2
 */
public class CustomReactiveElasticSearchTemplate implements ReactiveElasticsearchOperations, ApplicationContextAware {
	...
	public CustomReactiveElasticSearchTemplate(ReactiveElasticsearchClient client) {
		this(client, new MappingElasticsearchConverter(new SimpleElasticsearchMappingContext()));
	}

	public CustomReactiveElasticSearchTemplate(ReactiveElasticsearchClient client, ElasticsearchConverter converter) {
	...
	private CustomReactiveElasticSearchTemplate copy() {

		CustomReactiveElasticSearchTemplate copy = new CustomReactiveElasticSearchTemplate(client, converter);
	...
	/**
	 * Set the default {@link IndicesOptions} for {@link SearchRequest search
	 * requests}.
	 *
	 ...
	 * {@link ReactiveEntityCallbacks callbacks} like the
	 ...
	 * 
	 * @see
	 * org.springframework.data.elasticsearch.core.ReactiveDElasticsearchOperations#
	 * index(Object, IndexCoordinates)
	 */
	@Override
				...
				.flatMapMany(entities -> {
					...
					return doBulkOperation(entities.indexQueries(), BulkOptions.defaultOptions(), index) //
							.index().flatMap(indexAndResponse -> {
								...
								updateIndexedObject(savedEntity, IndexedObjectInformation.of(response.getId(),
	...
	private <T> T updateIndexedObject(T entity, IndexedObjectInformation indexedObjectInformation) {
		...
		if (persistentEntity != null) {
			...
			if (indexedObjectInformation.getSeqNo() != null && indexedObjectInformation.getPrimaryTerm() != null
				...
				ElasticsearchPersistentProperty seqNoPrimaryTermProperty = persistentEntity
						.getSeqNoPrimaryTermProperty();
				propertyAccessor.setProperty(seqNoPrimaryTermProperty, new SeqNoPrimaryTerm(
						indexedObjectInformation.getSeqNo(), indexedObjectInformation.getPrimaryTerm()));
			}
	 ...
	 *
	 * @param request the already prepared {@link IndexRequest} ready to be
	 *                executed.
	...
	protected Flux<BulkItemResponse> doBulkOperation(List<?> queries, BulkOptions bulkOptions, IndexCoordinates index) {
		...
		return client.bulk(bulkRequest) //
				.onErrorMap(e -> new UncategorizedElasticsearchException(
						"Error while bulk for request: " + bulkRequest.toString(), e)) //
	 ...
	 * Customization hook on the actual execution result {@link Publisher}. <br />
	 *
	 * @param request the already prepared {@link DeleteRequest} ready to be
	 *                executed.
	 * @return a {@link Mono} emitting the result of the operation.
	 */
	 ...
	 * Customization hook on the actual execution result {@link Publisher}. <br />
	 *
	 * @param request the already prepared {@link DeleteByQueryRequest} ready to be
	 *                executed.
	 * @return a {@link Mono} emitting the result of the operation.
	 */
	...
	/**
	 * Customization hook to modify a generated {@link DeleteRequest} prior to its
	 * execution. Eg. by setting the {@link WriteRequest#setRefreshPolicy(String)
	...
	/**
	 * Customization hook to modify a generated {@link DeleteByQueryRequest} prior
	 * to its execution. Eg. by setting the
	...
	/**
	 * Customization hook to modify a generated {@link IndexRequest} prior to its
	 * execution. Eg. by setting the {@link WriteRequest#setRefreshPolicy(String)
	 * refresh policy} if applicable.
	 *
	...
	/**
	 * Pre process the write request before it is sent to the server, eg. by setting
	 * the {@link WriteRequest#setRefreshPolicy(String) refresh policy} if
	 * applicable.
	 *
	 ...
	 * Customization hook on the actual execution result {@link Publisher}. <br />
	 *
	 * @param request the already prepared {@link SearchRequest} ready to be
	 *                executed.
	 * @return a {@link Flux} emitting the result of the operation converted to
	 *         {@link SearchDocument}s.
	 */
	 ...
	 * @param request the already prepared {@link SearchRequest} ready to be
	 *                executed.
	 * @return a {@link Mono} emitting the result of the operation converted to s
	 *         {@link SearchDocumentResponse}.
	 */
	...
	/**
	 * Customization hook to modify a generated {@link SearchRequest} prior to its
	 * execution. Eg. by setting the
	 * {@link SearchRequest#indicesOptions(IndicesOptions) indices options} if
	 * applicable.
	 *
	...
	protected Mono<String> getClusterVersion() {
		try {
			return Mono.from(execute(client -> client.info()))
	...
	protected ReactiveElasticsearchClient.Indices getIndicesClient() {
		...
		throw new UncategorizedElasticsearchException(
				"No ReactiveElasticsearchClient.Indices implementation available");
	}
	...
	protected ReactiveElasticsearchClient.Cluster getClusterClient() {
		...
		throw new UncategorizedElasticsearchException(
				"No ReactiveElasticsearchClient.Cluster implementation available");
	}

	/**
	 * translates an Exception if possible. Exceptions that are no
	 * {@link RuntimeException}s are wrapped in a RuntimeException
	 *
	...
	public ReactiveElasticsearchOperations withRouting(RoutingResolver routingResolver) {
		...
		CustomReactiveElasticSearchTemplate copy = copy();
		copy.setRoutingResolver(routingResolver);
		return copy;
	}
	// endregion
	...
	private class Entities<T> {
		...
		private List<IndexQuery> indexQueries() {
			return entities.stream().map(CustomReactiveElasticSearchTemplate.this::getIndexQuery)
					.collect(Collectors.toList());
		}

		private T entityAt(long index) {
			// it's safe to cast to int because the original indexed colleciton was fitting
			// in memory
