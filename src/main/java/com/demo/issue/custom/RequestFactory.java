...
/**
 * Factory class to create Elasticsearch request instances from Spring Data
 * Elasticsearch query objects.
 *
...
class RequestFactory {
	...
	/**
	 * The version for the transport client needs a different
	 * PutIndexTemplateRequest class
	 */
	...
	public HighlightBuilder highlightBuilder(Query query) {
		HighlightBuilder highlightBuilder = query.getHighlightQuery().map(HighlightQuery::getHighlightBuilder)
				.orElse(null);
	...
	private SearchRequest prepareSearchRequest(Query query, @Nullable Class<?> clazz,
	...
	private SearchRequestBuilder prepareSearchRequestBuilder(Query query, Client client, @Nullable Class<?> clazz,
		...
		query.getRescorerQueries()
				.forEach(rescorer -> searchRequestBuilder.addRescorer(getQueryRescorerBuilder(rescorer)));
	...
	private SortBuilder<?> getSortBuilder(Sort.Order order, @Nullable ElasticsearchPersistentEntity<?> entity) {
		...
		if (ScoreSortBuilder.NAME.equals(order.getProperty())) {
			...
			if (order instanceof GeoDistanceOrder) {
				...
				GeoDistanceSortBuilder sort = SortBuilders.geoDistanceSort(fieldName,
						geoDistanceOrder.getGeoPoint().getLat(), geoDistanceOrder.getGeoPoint().getLon());
	...
	public UpdateRequest updateRequest(UpdateQuery query, IndexCoordinates index) {
		...
		if (query.getScript() != null) {
			...
			Script script = new Script(getScriptType(query.getScriptType()), query.getLang(), query.getScript(),
					params);
	...
	public UpdateRequestBuilder updateRequestBuilderFor(Client client, UpdateQuery query, IndexCoordinates index) {
		...
		String indexName = index.getIndexName();
		UpdateRequestBuilder updateRequestBuilder = client.prepareUpdate(indexName, IndexCoordinates.TYPE,
				query.getId());
		...
		if (query.getRefreshPolicy() != null) {
			updateRequestBuilder
					.setRefreshPolicy(RequestFactory.toElasticsearchRefreshPolicy(query.getRefreshPolicy()));
		}
